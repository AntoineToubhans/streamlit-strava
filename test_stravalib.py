...
def get_stream_df_from_strava_activity(activity):
    activity_streams = client.get_activity_streams(activity_id=activity.id, types=STRAVA_STREAM_TYPES)
...
frac_z2 = ((streams_df.velocity_smooth >= Z_1_2) & (streams_df.velocity_smooth <= Z_2_3)).mean()
frac_z3 = ((streams_df.velocity_smooth >= Z_2_3) & (streams_df.velocity_smooth <= Z_3_4)).mean()
frac_z4 = ((streams_df.velocity_smooth >= Z_3_4) & (streams_df.velocity_smooth <= Z_4_5)).mean()
frac_z5 = ((streams_df.velocity_smooth >= Z_4_5) & (streams_df.velocity_smooth <= Z_5_6)).mean()
frac_z6 = ((streams_df.velocity_smooth >= Z_5_6) & (streams_df.velocity_smooth <= Z_6_7)).mean()
